...
def compute_dgst_t_batch_from_captures(
    *,
    ...
    visual_start: int,
    visual_end: int,
    visual_grid: Sequence[int] | None = None,
    image_token_id: int,
    ...
    four_gate_spatial_cost_semantic_weight: float = 1.0,
    four_gate_spatial_cost_distance_weight: float = 1.0,
    ...
    jffn_entropy_beta_by_layer: Sequence[float] | None = None,
    jffn_max_increment_gib: float = 1.5,
    jffn_chunk_size: int | None = None,
    jffn_force_full_parallel: bool = False,
    jffn_validate_linearity: bool = False,
    jffn_finite_difference_eta: float | None = None,
    jffn_parity_chunk_size: int | None = None,
    jffn_second_round_diagnostics: bool = False,
    ...
    if _normalize_target_gate_mode(target_gate_mode) == "four_gate":
        normalized_source_modes = _normalize_four_gate_source_modes(source_modes)
        jffn_payload = None
        if any(mode.startswith("jffn") for mode in normalized_source_modes):
            if "jffn_entropy_matched" in normalized_source_modes and (
                jffn_entropy_beta_by_layer is None
            ):
                raise ValueError(
                    "jffn_entropy_matched requires frozen "
                    "jffn_entropy_beta_by_layer values."
                )
            from features.visual_ffn_jacobian import build_jffn_source_payload

            max_increment_gib = float(jffn_max_increment_gib)
            if not math.isfinite(max_increment_gib) or max_increment_gib <= 0.0:
                raise ValueError("jffn_max_increment_gib must be finite and positive")
            jffn_payload = build_jffn_source_payload(
                model=model,
                captures=captures,
                prediction_positions=pred_positions,
                visual_start=int(visual_start),
                visual_end=int(visual_end),
                entropy_beta_by_layer=jffn_entropy_beta_by_layer,
                max_increment_bytes=int(max_increment_gib * 1024**3),
                fixed_chunk_size=(
                    int(jffn_chunk_size)
                    if jffn_chunk_size is not None
                    else None
                ),
                force_full_parallel=bool(jffn_force_full_parallel),
                validate_linearity=bool(jffn_validate_linearity),
                finite_difference_eta=jffn_finite_difference_eta,
                parity_chunk_size=jffn_parity_chunk_size,
                include_second_round_diagnostics=bool(
                    jffn_second_round_diagnostics
                ),
            ...
            if jffn_second_round_only:
                if not jffn_second_round_diagnostics:
                    raise ValueError(
                        "jffn_second_round_only requires "
                        "jffn_second_round_diagnostics=True"
                    )
                second_round_results: list[dict[str, Any]] = []
                for target_offset in range(len(target_ids)):
                    second_round_result = {
                            "dgst_t_jffn_energy_per_layer": torch.stack(
                                [
                                    entry["energy"][target_offset]
                                    for entry in jffn_payload
                                ]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_source_jffn_dist_per_layer": torch.stack(
                                [
                                    entry["distributions"]["jffn"][target_offset]
                                    for entry in jffn_payload
                                ]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_second_round_attention_dist_per_layer": torch.stack(
                                [
                                    entry["second_round"]["attention_distribution"][
                                        target_offset
                                    ]
                                    for entry in jffn_payload
                                ]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_second_round_write_energy_per_layer": torch.stack(
                                [
                                    entry["second_round"]["write_energy"][target_offset]
                                    for entry in jffn_payload
                                ]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_second_round_write_dist_per_layer": torch.stack(
                                [
                                    entry["second_round"]["write_distribution"][
                                        target_offset
                                    ]
                                    for entry in jffn_payload
                                ]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_second_round_signed_q_per_layer": torch.stack(
                                [
                                    entry["second_round"]["signed_q"][target_offset]
                                    for entry in jffn_payload
                                ]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_second_round_q_conservation_error_per_layer": torch.stack(
                                [
                                    entry["second_round"][
                            ...
                            "dgst_t_second_round_cancellation_ratio_per_layer": torch.stack(
                                [
                                    entry["second_round"]["cancellation_ratio"][
                                        target_offset
                                    ]
                                    for entry in jffn_payload
                                ]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_jffn_input_norm_per_layer": torch.stack(
                                [entry["input_norm"][target_offset] for entry in jffn_payload]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_jffn_response_norm_per_layer": torch.stack(
                                [entry["response_norm"][target_offset] for entry in jffn_payload]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_jffn_gain_per_layer": torch.stack(
                                [entry["gain"][target_offset] for entry in jffn_payload]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_jffn_direction_cosine_per_layer": torch.stack(
                                [
                                    entry["direction_cosine"][target_offset]
                                    for entry in jffn_payload
                                ]
                            ).detach().to(device="cpu", dtype=torch.float32),
                            "dgst_t_jffn_attention_reconstruction_relative_error_per_layer": torch.tensor(
                                [
                            ...
                            "dgst_t_jffn_component_sum_relative_error_per_layer": torch.tensor(
                                [
                    ...
                    if jffn_union_target_distributions is not None:
                        union_entries = [
                            entry["second_round"]["union_aggregate"]
                            for entry in jffn_payload
                        ]
                        for field in (
                            "input_norm",
                            ...
                            dtype = (
                                torch.int64 if field == "union_size" else torch.float32
                            )
                            second_round_result[
                                f"dgst_t_union_aggregate_{field}_per_layer"
                            ] = torch.stack(
                                [entry[field][target_offset] for entry in union_entries]
                            ).detach().to(device="cpu", dtype=dtype)
                    second_round_results.append(second_round_result)
        ...
        return compute_four_gate_dgst_batch_from_captures(
            ...
            spatial_cost_semantic_weight=float(
                four_gate_spatial_cost_semantic_weight
            ),
            spatial_cost_distance_weight=float(
                four_gate_spatial_cost_distance_weight
            ),
...
def _union_cosine_diagnostics_enabled() -> bool:
    raw = os.environ.get("DGST_UNION_COSINE_DIAGNOSTICS", "0").strip().lower()
    if raw in {"1", "true", "yes", "on"}:
        return True
    if raw in {"0", "false", "no", "off"}:
        return False
    raise ValueError(
        "DGST_UNION_COSINE_DIAGNOSTICS must be one of "
        "1/0, true/false, yes/no, or on/off."
    )


def _union_cosine_diagnostics_only_enabled() -> bool:
    raw = os.environ.get("DGST_UNION_COSINE_DIAGNOSTICS_ONLY", "0").strip().lower()
    if raw in {"1", "true", "yes", "on"}:
        return True
    if raw in {"0", "false", "no", "off"}:
        return False
    raise ValueError(
        "DGST_UNION_COSINE_DIAGNOSTICS_ONLY must be one of "
        "1/0, true/false, yes/no, or on/off."
    )


def _distribution_scalar_summary(values: torch.Tensor) -> dict[str, torch.Tensor]:
    flat = torch.nan_to_num(
        values.float().flatten(), nan=0.0, posinf=1.0, neginf=-1.0
    )
    if flat.numel() == 0:
    ...
    quantiles = torch.quantile(
        flat, torch.tensor([0.1, 0.5, 0.9], device=flat.device)
    )
    return {
        "count": torch.tensor(float(flat.numel()), device=flat.device),
        "sum": flat.sum(),
        "sum_sq": torch.square(flat).sum(),
        "mean": flat.mean(),
...
def _prefixed_scalar_summary(
    prefix: str,
    values: torch.Tensor,
) -> dict[str, torch.Tensor]:
    return {
        f"{prefix}_{name}": value
        for name, value in _distribution_scalar_summary(values).items()
    }


def _union_cosine_geometry_summary(
    *,
    source_dist: torch.Tensor,
    target_dist: torch.Tensor,
    ...
    """Reduce the exact union-support cosine geometry without retaining states."""
    if support.numel() == 0:
        raise ValueError("Union cosine diagnostics require non-empty support.")
    source_topk, target_topk = _source_target_topk_indices(
        source=source_dist,
        target=target_dist,
        top_k=int(transport_top_k),
    )
    intersection_size = torch.isin(source_topk, target_topk).sum()
    selected_states = visual_states.index_select(0, support).float()
    selected_normalized = F.normalize(selected_states, dim=-1)
    vv_matrix = torch.matmul(selected_normalized, selected_normalized.transpose(0, 1))
    ...
    vv_cosine = vv_matrix[pair_indices[0], pair_indices[1]].clamp(-1.0, 1.0)
    qv_cosine = F.cosine_similarity(
        prediction_state.float().unsqueeze(0), selected_states, dim=-1
    ).clamp(-1.0, 1.0)

    visual_mean = visual_states.float().mean(dim=0, keepdim=True)
    centered_selected = selected_states - visual_mean
    centered_prediction = prediction_state.float().unsqueeze(0) - visual_mean
    centered_normalized = F.normalize(centered_selected, dim=-1)
    centered_vv_matrix = torch.matmul(
        centered_normalized, centered_normalized.transpose(0, 1)
    )
    centered_vv = centered_vv_matrix[
        pair_indices[0], pair_indices[1]
    ].clamp(-1.0, 1.0)
    centered_qv = F.cosine_similarity(
        centered_prediction, centered_selected, dim=-1
    ).clamp(-1.0, 1.0)

    local_source = _renormalize(source_dist.index_select(0, support))
    local_target = _renormalize(target_dist.index_select(0, support))
    total_variation = 0.5 * torch.abs(local_source - local_target).sum()
    cosine_cost = (1.0 - vv_cosine).clamp_min(0.0)
    sqrt_cost = torch.sqrt((cosine_cost / 2.0).clamp_min(0.0))
    cosine_cost_mean = (
        cosine_cost.mean()
        if cosine_cost.numel()
        else torch.zeros((), device=support.device)
    )
    sqrt_cost_mean = (
        sqrt_cost.mean()
        if sqrt_cost.numel()
    ...
    qv_prob = torch.softmax(qv_cosine / float(tau), dim=0)
    if qv_prob.numel() <= 1:
        qv_normalized_entropy = torch.zeros((), device=support.device)
    else:
        qv_normalized_entropy = -(
            qv_prob * torch.log(qv_prob.clamp_min(EPS))
        ).sum() / math.log(int(qv_prob.numel()))
    qv_median = torch.quantile(qv_cosine, 0.5)

    result = {
        "support_size": torch.tensor(float(support.numel()), device=support.device),
        "source_topk_size": torch.tensor(
            float(source_topk.numel()), device=support.device
        ),
        "target_topk_size": torch.tensor(
            float(target_topk.numel()), device=support.device
        ),
        "intersection_size": intersection_size.float(),
        "total_variation": total_variation,
        "constant_risk_sqrt": total_variation * sqrt_cost_mean,
        "constant_risk_cosine": total_variation * cosine_cost_mean,
        "qv_softmax_normalized_entropy": qv_normalized_entropy,
        "qv_softmax_max_over_uniform": qv_prob.max() * qv_prob.numel(),
        "qv_top1_minus_median": qv_cosine.max() - qv_median,
        "qv_top1_minus_bottom": qv_cosine.max() - qv_cosine.min(),
        "vv_hist": torch.histc(
            vv_cosine, bins=int(hist_bins), min=-1.0, max=1.0
        ),
        "qv_hist": torch.histc(
            qv_cosine, bins=int(hist_bins), min=-1.0, max=1.0
        ),
    }
    result.update(_prefixed_scalar_summary("vv", vv_cosine))
    result.update(_prefixed_scalar_summary("vv_centered", centered_vv))
    result.update(_prefixed_scalar_summary("qv", qv_cosine))
    result.update(_prefixed_scalar_summary("qv_centered", centered_qv))
    result.update(_prefixed_scalar_summary("sqrt_cost", sqrt_cost))
    result.update(_prefixed_scalar_summary("cosine_cost", cosine_cost))
    return result
...
def compute_four_gate_dgst_batch_from_captures(
    *,
    ...
    source_modes: Sequence[str] | str | None = None,
    compute_dual_scope: bool = False,
    ...
    spatial_cost_semantic_weight: float = 1.0,
    spatial_cost_distance_weight: float = 1.0,
    ...
    prompt_cafe_temperature: float = 10.0,
    prompt_cafe_layer: int = 22,
    jffn_payload: Sequence[Mapping[str, Any]] | None = None,
    ...
    ``source_modes`` compares the original ``cos(o_ffn(q_t), hmid(v_i))``
    source marginal with ``cos(o_ffn(q_t), hpre(v_i))`` without mixing the
    two scores into one distribution.
    ``sqrt_matched_state_plus_spatial`` adds normalized 2-D visual-grid
    distance to the existing sqrt-cosine semantic cost. Prompt-involving
    pairs receive zero spatial cost, and both coefficients default to one.
    ...
    if any(mode.startswith("jffn") for mode in normalized_source_modes):
        if active_modes != ("vv",):
            raise ValueError(
                "JFFN-P is defined only on visual support; support_modes must "
                "be exactly ['vv']."
            )
        if jffn_payload is None:
            raise ValueError("JFFN source mode requires a precomputed jffn_payload")
    ...
    for scope_name, support_positions in scopes:
        scoped_results[scope_name] = _compute_four_gate_single_scope_from_captures(
            ...
            source_modes=source_modes,
            jffn_payload=(jffn_payload if scope_name == "visual" else None),
            source_tau_values=source_tau_values,
            transport_top_k_values=transport_top_k_values,
            spatial_cost_semantic_weight=float(
                spatial_cost_semantic_weight
            ),
            spatial_cost_distance_weight=float(
                spatial_cost_distance_weight
            ),
    ...
    for result in results:
        if len(active_modes) > 1 or active_modes[0] != "vv":
            profile_version = (
                "v2_source_compare"
                if "hpre_cos" in result.get("dgst_t_source_modes", ())
                else "v1"
            )
            result["dgst_t_profile"] = (
                f"four_gate_{profile_slug}_{profile_version}"
            )
...
def _compute_four_gate_single_scope_from_captures(
    *,
    ...
    normalized_source_modes = _normalize_four_gate_source_modes(source_modes)
    jffn_source_modes = tuple(
        mode for mode in normalized_source_modes if mode.startswith("jffn")
    )
    if jffn_source_modes:
        if str(support_scope) != "visual":
            raise ValueError("JFFN-P cannot be used with prompt support")
        if source_tau_values is not None or transport_top_k_values is not None:
            raise ValueError(
                "JFFN-P comparison does not support source tau/Top-K sweeps; "
                "use the fixed preregistered Union Top-K setting."
            )
        if jffn_payload is None or len(jffn_payload) != len(captures):
            raise ValueError(
                "jffn_payload must contain exactly one entry per decoder layer"
            )
    compute_union_cosine_diagnostics = _union_cosine_diagnostics_enabled()
    union_cosine_diagnostics_only = _union_cosine_diagnostics_only_enabled()
    if union_cosine_diagnostics_only and not compute_union_cosine_diagnostics:
        raise ValueError(
            "DGST_UNION_COSINE_DIAGNOSTICS_ONLY requires "
            "DGST_UNION_COSINE_DIAGNOSTICS=1."
        )
    ...
    spatial_cost_mode = "sqrt_cosine_matched_state_plus_spatial"
    support_spatial_cost = None
    if spatial_cost_mode in normalized_cost_modes:
        semantic_weight = float(spatial_cost_semantic_weight)
        distance_weight = float(spatial_cost_distance_weight)
        if (
            not math.isfinite(semantic_weight)
            or semantic_weight < 0.0
            or not math.isfinite(distance_weight)
            or distance_weight < 0.0
        ):
            raise ValueError(
                "spatial cost weights must be finite non-negative values."
            )
        if semantic_weight == 0.0 and distance_weight == 0.0:
            raise ValueError("At least one spatial cost weight must be positive.")
        support_spatial_cost = _normalized_visual_spatial_distance_matrix(
            visual_grid=visual_grid,
            visual_start=int(visual_start),
            visual_end=int(visual_end),
        ...
        records.append(
            {
                "attention": [],
                "sources": {
                    source_mode: [] for source_mode in normalized_source_modes
                },
                "gates": {
                    method: []
                ...
                "problems": {
                    (source_mode, method, active_cost): []
                    for source_mode in normalized_source_modes
                    for method in methods
                    for active_cost in normalized_cost_modes
                },
                "capped_problems": {
                    (alpha_slug, source_mode, method, active_cost): []
                ...
                "union_cosine_diagnostics": (
                    {
                        (source_mode, method): []
                        for source_mode in normalized_source_modes
                        for method in methods
                    }
                    if compute_union_cosine_diagnostics
                    else None
                ),
            }
        )
    ...
    for layer_index, capture in enumerate(captures):
        ...
        compact_capture = build_compact_four_gate_layer_capture(
            ...
            source_modes=normalized_source_modes,
        )
        ...
        layer_sweep_problems = {
            (variant_slug, source_mode, method, active_cost): []
            for variant_slug, _tau_value, _top_k_value in sweep_specs
            for source_mode in normalized_source_modes
            for method in methods
            for active_cost in normalized_cost_modes
        ...
        layer_sweep_capped_problems = (
            {
                (tau_slug, alpha_slug, source_mode, method, active_cost): []
                for tau_slug, _tau_value in sweep_tau_specs
                for alpha_slug, _alpha_value in capped_alpha_specs
                for source_mode in normalized_source_modes
                for method in methods
        ...
        preparation_cache = _FourGatePreparationCache(
            ...
            spatial_cost_matrix=support_spatial_cost,
            spatial_cost_semantic_weight=float(spatial_cost_semantic_weight),
            spatial_cost_distance_weight=float(spatial_cost_distance_weight),
        )
        ...
        for target_offset, prediction_position in enumerate(pred_positions):
            ...
            source_dists = {
                "hmid_cos": compact_capture["source_dist"][target_offset],
            }
            if "hpre_cos" in normalized_source_modes:
                hpre_source_matrix = compact_capture["source_hpre_cos_dist"]
                if hpre_source_matrix is None:
                    raise RuntimeError(
                        "hpre_cos source mode requires its compact source matrix."
                    )
                source_dists["hpre_cos"] = hpre_source_matrix[target_offset]
            for jffn_mode in jffn_source_modes:
                layer_distributions = jffn_payload[layer_index].get(
                    "distributions", {}
                )
                if jffn_mode not in layer_distributions:
                    raise RuntimeError(
                        f"Layer {layer_index} JFFN payload has no {jffn_mode!r} "
                        "distribution."
                    )
                matrix = layer_distributions[jffn_mode]
                expected = (len(target_ids), len(normalized_support_positions))
                if tuple(matrix.shape) != expected:
                    raise ValueError(
                        f"Layer {layer_index} {jffn_mode} shape "
                        f"{tuple(matrix.shape)} != {expected}."
                    )
                source_dists[jffn_mode] = matrix[target_offset].to(
                    device=attention_support.device,
                    dtype=torch.float32,
                )
            source_dists = {
                mode: source_dists[mode] for mode in normalized_source_modes
            }
            source_cache_keys = {
                mode: (
                    "source",
                ...
                for mode in normalized_source_modes
            }
            ...
            if sweep_specs:
                ...
                source_scores = {
                    "hmid_cos": F.cosine_similarity(
                        source_update, visual_hmid, dim=-1
                    ),
                    "hpre_cos": F.cosine_similarity(
                        source_update, visual_hpre, dim=-1
                    ),
                }
                sweep_source_dist = {
                    (mode, tau_value): _source_distribution_from_scores(
                        source_scores[mode],
                        tau=tau_value,
                        mode="softmax",
                    )
                ...
                sweep_source_cache_keys = {
                    ...
                    for mode, tau_value in sweep_source_dist
                }
            state_views = {
            ...
            for source_mode, source_dist in source_dists.items():
                record["sources"][source_mode].append(source_dist)
            for method in methods:
                ...
                for source_mode, source_dist in source_dists.items():
                    source_cache_key = source_cache_keys[source_mode]
                    ...
                    if compute_union_cosine_diagnostics:
                        record["union_cosine_diagnostics"][
                            (source_mode, method)
                        ].append(
                            _union_cosine_geometry_summary(
                                source_dist=source_dist,
                                target_dist=target_dist,
                                support=support,
                                prediction_state=_prediction_state,
                                visual_states=cost_states,
                                transport_top_k=int(transport_top_k),
                                tau=float(tau),
                                hist_bins=UNION_COSINE_HIST_BINS,
                            )
                        )
                    if union_cosine_diagnostics_only:
                        continue
                    for active_cost in normalized_cost_modes:
                    ...
                    for variant_slug, tau_value, top_k_value in sweep_specs:
                        sweep_key = (source_mode, tau_value)
                        variant_source_dist = sweep_source_dist[sweep_key]
                        variant_source_key = sweep_source_cache_keys[sweep_key]
                    ...
                    if compute_capped_topmass_085:
                        for tau_slug, tau_value in sweep_tau_specs:
                            ...
                            variant_source_key = sweep_source_cache_keys[sweep_key]
                            for alpha_slug, alpha_value in capped_alpha_specs:
                                variant_capped_support = (
                                    preparation_cache.capped_union(
                                        source_key=variant_source_key,
                                        source=variant_source_dist,
                                        target_key=target_cache_key,
                                        target=target_dist,
    ...
    for target_offset, record in enumerate(records):
        ...
        risk_series = (
            {}
            if union_cosine_diagnostics_only
            else _solve_exact_emd_problem_series(record["problems"])
        )
        capped_risk_series = (
        ...
        result: dict[str, Any] = {
            "dgst_t_profile": (
                "four_gate_vv_v2_source_compare"
                if "hpre_cos" in normalized_source_modes
                else (
            ...
            "dgst_t_source_modes": list(normalized_source_modes),
            "dgst_t_source_definitions": {
                "hmid_cos": (
                    "softmax(cos(o_ffn(q_t),hmid(support_i))/tau)"
                ),
                **(
                    {
                        "hpre_cos": (
                            "softmax(cos(o_ffn(q_t),hpre(support_i))/tau)"
                        )
                    }
                    if "hpre_cos" in normalized_source_modes
                    else {}
                ...
                **(
                    {
                        "jffn": (
                            "norm(J_FFN(Norm(hmid(q_t))) a_visual_j)_2 "
                            "renormalized_over_visual_tokens"
                        )
                    }
                    if "jffn" in normalized_source_modes
                    else {}
                ),
                **(
                    {
                        "jffn_entropy_matched": (
                            "softmax(log(jffn_energy+eps)/frozen_train_beta_l)"
                        )
                    }
                    if "jffn_entropy_matched" in normalized_source_modes
                    else {}
                ),
            },
            "dgst_t_mad_axis": {
            ...
            "dgst_t_source_dist_per_layer": torch.stack(
                record["sources"]["hmid_cos"], dim=0
            ).detach().to(device="cpu", dtype=torch.float32),
        }
        if "hpre_cos" in normalized_source_modes:
            result["dgst_t_source_hpre_cos_dist_per_layer"] = torch.stack(
                record["sources"]["hpre_cos"], dim=0
            ).detach().to(device="cpu", dtype=torch.float32)
        for jffn_mode in jffn_source_modes:
            result[f"dgst_t_source_{jffn_mode}_dist_per_layer"] = torch.stack(
                record["sources"][jffn_mode], dim=0
            ).detach().to(device="cpu", dtype=torch.float32)
        if jffn_source_modes:
            result["dgst_t_jffn_energy_per_layer"] = torch.stack(
                [
                    entry["energy"][target_offset]
                    for entry in jffn_payload
                ],
            ...
            for field in scalar_fields:
                result[f"dgst_t_jffn_{field}_per_layer"] = torch.stack(
                    [entry[field][target_offset] for entry in jffn_payload]
                ).detach().to(device="cpu", dtype=torch.float32)
            for jffn_mode in jffn_source_modes:
                statistic_names = tuple(
                    jffn_payload[0]["distribution_statistics"][jffn_mode]
                )
                for statistic_name in statistic_names:
                    result[
                        f"dgst_t_source_{jffn_mode}_{statistic_name}_per_layer"
                    ] = torch.stack(
                        [
                            entry["distribution_statistics"][jffn_mode][
                                statistic_name
                            ][target_offset]
                            for entry in jffn_payload
                        ]
                    ).detach().to(device="cpu", dtype=torch.float32)
            result["dgst_t_jffn_attention_reconstruction_relative_error_per_layer"] = torch.tensor(
                [entry["reconstruction_relative_error"] for entry in jffn_payload],
                dtype=torch.float32,
            )
            result["dgst_t_jffn_attention_reconstruction_cosine_per_layer"] = torch.tensor(
                [entry["reconstruction_cosine"] for entry in jffn_payload],
                dtype=torch.float32,
            )
            ...
            result["dgst_t_jffn_chunk_size_per_layer"] = torch.tensor(
                [entry["jvp_chunk_size"] for entry in jffn_payload],
                dtype=torch.int32,
            )
            result["dgst_t_jffn_parallel_chunk_relative_error_per_layer"] = torch.tensor(
                [entry["parallel_chunk_relative_error"] for entry in jffn_payload],
                dtype=torch.float32,
            )
            result["dgst_t_jffn_validation_computed"] = bool(
                all(entry.get("validation_computed", False) for entry in jffn_payload)
            )
        if compute_union_cosine_diagnostics:
            diagnostic_branches: dict[str, dict[str, torch.Tensor]] = {}
            for source_mode in normalized_source_modes:
                for method in methods:
                    entries = record["union_cosine_diagnostics"][
                        (source_mode, method)
                    ]
                    if len(entries) != len(captures):
                        raise AssertionError(
                            "Union cosine diagnostic layer count does not match captures."
                        )
                    branch_name = f"{source_mode}__{method}"
                    diagnostic_branches[branch_name] = {
                        key: torch.stack([entry[key] for entry in entries], dim=0)
                        .detach()
                        .to(device="cpu")
                        for key in entries[0]
                    }
            result["dgst_t_union_cosine_diagnostics"] = {
                "support_definition": (
                    "union(top_floor(transport_top_k/2,source),"
                    "top_floor(transport_top_k/2,target))"
                ),
                "transport_top_k": int(transport_top_k),
                "hist_edges": torch.linspace(
                    -1.0,
                    1.0,
                    UNION_COSINE_HIST_BINS + 1,
                    dtype=torch.float32,
                ),
                "diagnostics_only": bool(union_cosine_diagnostics_only),
                "by_branch": diagnostic_branches,
            }
        if spatial_cost_mode in normalized_cost_modes:
            result.update(
                {
                    "dgst_t_spatial_cost_semantic_weight": float(
                        spatial_cost_semantic_weight
                    ),
                    "dgst_t_spatial_cost_distance_weight": float(
                        spatial_cost_distance_weight
                    ),
                    "dgst_t_spatial_cost_visual_grid": [
                        int(value)
                        for value in (
                            visual_grid if visual_grid is not None else ()
                        )
                    ],
                    "dgst_t_spatial_cost_distance_normalization": (
                        "euclidean_grid_distance_divided_by_grid_diagonal"
                    ),
                    "dgst_t_spatial_cost_prompt_policy": (
                        "zero_if_either_token_is_nonvisual"
                    ),
                }
            )
        if compute_capped_topmass_085:
        ...
        for method in methods:
            ...
            for source_mode in normalized_source_modes:
                source_prefix = _four_gate_source_risk_prefix(source_mode)
                for active_cost in normalized_cost_modes:
                    risk_suffix = _four_gate_risk_suffix(active_cost, state_name)
                    if not union_cosine_diagnostics_only:
                        result[
                            f"dgst_t_{method}_{source_prefix}{risk_suffix}_per_layer"
                        ] = torch.tensor(
                            risk_series[(source_mode, method, active_cost)],
                            dtype=torch.float32,
                        )
                    if compute_capped_topmass_085:
                        for alpha_slug, _alpha_value in capped_alpha_specs:
                            result[
                                f"dgst_t_{method}_{source_prefix}{risk_suffix}_"
        ...
        if sweep_specs:
            ...
            for variant_slug, tau_value, top_k_value in sweep_specs:
                ...
                for method in methods:
                    state_name = _target_comparison_state(method)
                    for source_mode in normalized_source_modes:
                        source_prefix = _four_gate_source_risk_prefix(
                            source_mode
                        )
                        for active_cost in normalized_cost_modes:
                            ...
                            risk_key = (
                                f"dgst_t_{method}_{source_prefix}"
                                f"{risk_suffix}_per_layer"
                            )
                            ...
                            if compute_capped_topmass_085:
                                for alpha_slug, _alpha_value in capped_alpha_specs:
                                    capped_risk_key = (
                                        f"dgst_t_{method}_{source_prefix}"
                                        f"{risk_suffix}_{alpha_slug}_per_layer"
                                    )
...
def build_compact_four_gate_layer_capture(
    *,
    ...
    result = {
        ...
        "source_dist": source_dist,
        "source_hpre_cos_dist": source_hpre_cos_dist,
        "hpre_raw_target_logits": (
...
def _normalize_four_gate_source_modes(
    values: Sequence[str] | str | None,
) -> tuple[str, ...]:
    """Normalize compact-path source marginals in deterministic order."""
    if values is None:
        return ("hmid_cos",)
    ...
    aliases = {
        ...
        "jffn_p": "jffn",
        "new_jffn": "jffn",
        "jffn_entropy_matched": "jffn_entropy_matched",
    ...
    if unknown:
        raise ValueError(
            f"Unknown four-gate source modes {unknown}; expected a subset of "
            f"{list(FOUR_GATE_SOURCE_MODES)}."
        )
    if not normalized:
        raise ValueError("At least one four-gate source mode must be enabled.")
    if "hmid_cos" not in normalized:
        normalized.insert(0, "hmid_cos")
    return tuple(mode for mode in FOUR_GATE_SOURCE_MODES if mode in normalized)


def _four_gate_source_risk_prefix(source_mode: str) -> str:
    mode = str(source_mode).strip().lower().replace("-", "_")
    if mode not in FOUR_GATE_SOURCE_MODES:
        raise ValueError(f"Unknown normalized four-gate source mode: {source_mode!r}")
...
def _normalized_visual_spatial_distance_matrix(
    *,
    visual_grid: Sequence[int] | None,
    visual_start: int,
    visual_end: int,
    ...
    """Return normalized 2-D distances, with zero cost for prompt pairs."""
    if visual_grid is None or len(visual_grid) != 2:
        raise ValueError(
            "sqrt_matched_state_plus_spatial requires visual_grid=(height, width)."
        )
    height, width = (int(value) for value in visual_grid)
    if height <= 0 or width <= 0:
        raise ValueError("visual_grid dimensions must be positive.")
    visual_count = int(visual_end) - int(visual_start)
    if height * width != visual_count:
        raise ValueError(
            "visual_grid does not match the visual token range: "
            f"grid={height}x{width}, tokens={visual_count}."
        )
    ...
    visual_mask = (positions >= int(visual_start)) & (
        positions < int(visual_end)
    )
    visual_support_indices = torch.nonzero(
        visual_mask, as_tuple=False
    ).flatten()
    distance = torch.zeros(
        (int(positions.numel()), int(positions.numel())),
        dtype=torch.float32,
        device=device,
    )
    if visual_support_indices.numel() == 0:
        return distance

    offsets = positions.index_select(0, visual_support_indices) - int(
        visual_start
    )
    coordinates = torch.stack(
        (
            torch.div(offsets, width, rounding_mode="floor"),
            torch.remainder(offsets, width),
        ),
        dim=-1,
    ).float()
    visual_distance = torch.cdist(coordinates, coordinates, p=2)
    grid_diagonal = math.hypot(max(height - 1, 0), max(width - 1, 0))
    if grid_diagonal > 0.0:
        visual_distance = visual_distance / float(grid_diagonal)
    distance[
        visual_support_indices.unsqueeze(1),
        visual_support_indices.unsqueeze(0),
    ] = visual_distance
    return distance
...
def _normalize_four_gate_cost_mode(value: str) -> str:
    ...
    if mode in {
        "sqrt_matched_state_plus_spatial",
        "sqrt_cosine_matched_state_plus_spatial",
        "sqrt_matched_state_spatial",
        "semantic_plus_spatial",
    }:
        return "sqrt_cosine_matched_state_plus_spatial"
    if mode in {
        "sqrt_matched_state",
    ...
    raise ValueError(
        ...
        "'sqrt_matched_state_plus_spatial' or "
...
def _four_gate_risk_suffix(cost_mode: str, state_name: str) -> str:
    ...
    if normalized == "sqrt_cosine_matched_state_plus_spatial":
        return f"risk_sqrt_{state_name}_plus_spatial"
...
class _FourGatePreparationCache:
    ...
    def __init__(
        ...
        *,
        enabled: bool,
        spatial_cost_matrix: torch.Tensor | None = None,
        spatial_cost_semantic_weight: float = 1.0,
        spatial_cost_distance_weight: float = 1.0,
    ) -> None:
        self.enabled = bool(enabled)
        self.spatial_cost_matrix = spatial_cost_matrix
        self.spatial_cost_semantic_weight = float(
            spatial_cost_semantic_weight
        )
        self.spatial_cost_distance_weight = float(
            spatial_cost_distance_weight
        )
        ...
        *,
        ...
        if not self.enabled:
            return _prepare_four_gate_cost_problem(
                ...
                support=support,
                spatial_cost_matrix=self.spatial_cost_matrix,
                spatial_cost_semantic_weight=self.spatial_cost_semantic_weight,
                spatial_cost_distance_weight=self.spatial_cost_distance_weight,
            )
        if support.numel() == 0:
            return None
...
def _prepare_four_gate_cost_problem(
    *,
    ...
    normalized = _normalize_four_gate_cost_mode(cost_mode)
    if normalized == "sqrt_cosine_matched_state_plus_spatial":
        if spatial_cost_matrix is None:
            raise ValueError(
                "sqrt_matched_state_plus_spatial requires a visual grid."
            )
        ...
        if problem is None:
            return None
        local_source, local_target, semantic_cost = problem
        local_spatial_cost = spatial_cost_matrix.index_select(
            0, support
        ).index_select(1, support)
        combined_cost = (
            float(spatial_cost_semantic_weight) * semantic_cost
            + float(spatial_cost_distance_weight) * local_spatial_cost
        )
        return (
            local_source.detach().float().cpu().numpy().copy(),
            local_target.detach().float().cpu().numpy().copy(),
            combined_cost.detach().float().cpu().numpy().copy(),
        )
    if normalized == "geo_stateupd_lu1":
