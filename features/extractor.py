...
from utils.inslen_targeting import INSLEN_TARGET_PROTOCOL
...
def _base_token_record(
    *,
    ...
    return {
        ...
        **_targeting_metadata(span),
    }


def _targeting_metadata(span: Mapping[str, object]) -> dict[str, object]:
    resolution = span.get("inslen_official_resolution") or {}
    if not isinstance(resolution, Mapping):
        resolution = {}
    return {
        "target_locator_protocol": str(
            span.get("target_locator_protocol")
            or "inslen_official_first_token_first_occurrence"
        ),
        "official_detected_word": str(
            span.get("official_detected_word")
            or resolution.get("detected_word")
            or ""
        ),
        "official_candidate_surface": resolution.get("candidate_surface"),
        "official_resolver_branch": resolution.get("resolver_branch"),
    }
...
def _resolve_causal_target(
    *,
    ...
    """Resolve the InsLen response token and the prefix that predicts it.
    ...
    if not raw_indices:
        raise ValueError(
            f"Image {image_id}: InsLen object has no resolved token_indices"
        )
    try:
    ...
    declared_target_token_id = span.get("target_token_id")
    is_inslen_target = (
        str(span.get("target_locator_protocol") or "")
        == INSLEN_TARGET_PROTOCOL
    )
    if is_inslen_target and declared_target_token_id is None:
        raise ValueError(
            f"Image {image_id}: InsLen target is missing target_token_id"
        )
    if (
        declared_target_token_id is not None
        and int(declared_target_token_id) != target_token_id
    ):
        raise ValueError(
            f"Image {image_id}: InsLen target ID mismatch at response index "
            f"{first_index}: labeling={declared_target_token_id}, "
            f"generation={target_token_id}"
        )
    prefix_token_ids = [
...
def _build_four_gate_feature_record(
    *,
    ...
    configured_cost_modes = tuple(
        dgst_t.get("dgst_t_cost_modes") or [dgst_t["dgst_t_cost"]]
    )
    source_modes = tuple(dgst_t.get("dgst_t_source_modes") or ["hmid_cos"])
    has_hpre_source = "hpre_cos" in source_modes
    jffn_source_modes = tuple(
        mode for mode in source_modes if str(mode).startswith("jffn")
    )
    has_spatial_cost = (
        "sqrt_cosine_matched_state_plus_spatial" in configured_cost_modes
    )
    required_metadata = (
        "dgst_t_profile",
        "dgst_t_mad_axis",
        ...
        *(
            (
                "dgst_t_spatial_cost_semantic_weight",
                "dgst_t_spatial_cost_distance_weight",
    ...
    cost_modes = configured_cost_modes
    ...
    required_matrices = (
        (
            ...
            *(
                f"dgst_t_source_{source_mode}_dist_per_layer"
                for source_mode in source_modes
                if source_mode != "hmid_cos"
            ),
        )
        if has_vv_scope
        else ()
    )
    jffn_diagnostic_keys = (
        (
            "dgst_t_jffn_energy_per_layer",
            "dgst_t_jffn_input_norm_per_layer",
            "dgst_t_jffn_response_norm_per_layer",
            "dgst_t_jffn_gain_per_layer",
            "dgst_t_jffn_direction_cosine_per_layer",
            "dgst_t_jffn_response_sum_relative_error_per_layer",
            "dgst_t_jffn_local_fd_cosine_per_layer",
            "dgst_t_jffn_local_fd_relative_error_per_layer",
            "dgst_t_jffn_attention_reconstruction_relative_error_per_layer",
            "dgst_t_jffn_attention_reconstruction_cosine_per_layer",
            "dgst_t_jffn_chunk_size_per_layer",
            "dgst_t_jffn_parallel_chunk_relative_error_per_layer",
            *(
                f"dgst_t_source_{source_mode}_{statistic}_per_layer"
                for source_mode in jffn_source_modes
                for statistic in (
                    "normalized_entropy",
    ...
    if has_vv_scope:
        for method in methods:
            ...
            for source_mode in source_modes:
                source_prefix = (
                    "" if source_mode == "hmid_cos" else f"source_{source_mode}_"
                )
            ...
            for source_mode in source_modes:
                ...
                for cost_mode in cost_modes:
                    ...
                    vp_method_keys.append(
                        f"dgst_t_vp_{method}_{source_prefix}{risk_suffix}_per_layer"
                    )
                    if has_capped_topmass:
                        vp_method_keys.extend(
                            f"dgst_t_vp_{method}_{source_prefix}{risk_suffix}_"
                            f"{alpha_slug}_per_layer"
                            for alpha_slug in capped_slugs
                        )
            vp_method_keys.extend(
    ...
    vp_matrix_keys = (
        (
            ...
            "dgst_t_vp_source_dist_per_layer",
            *(("dgst_t_vp_source_hpre_cos_dist_per_layer",) if has_hpre_source else ()),
        )
        if has_vp_scope
        else ()
    )
        ...
        for method in methods:
            ...
            for source_mode in source_modes:
                ...
                for cost_mode in cost_modes:
                    risk_suffix = _four_gate_risk_suffix(cost_mode, state_name)
                    vpend_method_keys.append(
                        f"dgst_t_vpend_{method}_{source_prefix}{risk_suffix}_per_layer"
                    )
                    if has_capped_topmass:
                        vpend_method_keys.extend(
                            f"dgst_t_vpend_{method}_{source_prefix}{risk_suffix}_"
                            f"{alpha_slug}_per_layer"
                            for alpha_slug in capped_slugs
                        )
            vpend_method_keys.extend(
    ...
    vpend_matrix_keys = (
        (
            ...
            *(("dgst_t_vpend_source_hpre_cos_dist_per_layer",) if has_hpre_source else ()),
        )
        if has_vpend_scope
        else ()
    )
    ...
    feat = {
        "feature_schema_version": (
            ...
            "dgst-four-gate-v2-source-compare"
            if has_hpre_source
            else (
        ...
        **_targeting_metadata(span),
        ...
        "dgst_t_source_modes": list(source_modes),
    }
    if "dgst_t_source_definitions" in dgst_t:
        feat["dgst_t_source_definitions"] = dict(
            dgst_t["dgst_t_source_definitions"]
        )
    ...
    for key in jffn_diagnostic_keys:
        feat[key] = _compact_numpy(dgst_t[key], dtype=np.float32)
