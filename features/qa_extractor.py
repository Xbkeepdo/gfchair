...
def _compact_dgst(result: dict) -> dict:
    ...
    source_modes = tuple(result.get("dgst_t_source_modes") or ["hmid_cos"])
    ...
    for scope_name, raw_prefix, _ in scope_specs:
        ...
        if has_hpre_source:
            hpre_source_key = (
                f"dgst_t_{raw_prefix}source_hpre_cos_dist_per_layer"
            )
            if hpre_source_key not in result:
                raise KeyError(
                    f"Missing QA DGST hpre source field: {hpre_source_key}"
                )
            matrices_by_scope[scope_name]["source_hpre_cos_dist"] = (
                _as_float32_array(result[hpre_source_key])
            )
    ...
    for scope_name, raw_prefix, compact_prefix in scope_specs:
        for method in methods:
            ...
            method_prefix = f"dgst_t_{raw_prefix}{method}_"
            for key, value in result.items():
                if not (
                    key.startswith(method_prefix)
                ...
                if not (
                    component.startswith("risk_")
                    or component.startswith("source_hpre_cos_risk_")
                ):
                    continue
                if component == f"risk_sqrt_{state}":
                    component = "risk_sqrt_matched_state"
                elif component == f"risk_sqrt_{state}_plus_spatial":
                    component = "risk_sqrt_matched_state_plus_spatial"
                elif component == f"risk_cosine_{state}":
                    component = "risk_cosine_matched_state"
                elif component == f"source_hpre_cos_risk_sqrt_{state}":
                    component = "source_hpre_cos_risk_sqrt_matched_state"
                elif component == f"source_hpre_cos_risk_cosine_{state}":
                    component = "source_hpre_cos_risk_cosine_matched_state"
