...
def chunked_eager_attention_forward(
    ...
    **_kwargs,
    ...
    def weights_for(query_slice: torch.Tensor, start: int, end: int) -> torch.Tensor:
        ...
        # Decoder self-attention is causal.  Normally the model supplies the
        # 4-D causal mask, but some Transformers/Qwen paths elide it based on
        # the registered attention implementation.  A custom implementation
        # must therefore enforce causality itself as well.  Applying it twice
        # is idempotent and prevents a shared full-caption row from seeing
        ...
        ) + max(key_length - query_length, 0)
        key_positions = torch.arange(
            key_length, dtype=torch.long, device=query_slice.device
        )
    ...
    selected_key_positions = torch.arange(
        key_length, dtype=torch.long, device=query.device
    )
    selected_absolute_positions = position_index + max(
        key_length - query_length, 0
    )
    selected_future = (
        selected_key_positions.unsqueeze(0)
        > selected_absolute_positions.unsqueeze(1)
    )
    selected_weights = selected_weights.masked_fill(
        selected_future.unsqueeze(0).unsqueeze(0),
        torch.finfo(selected_weights.dtype).min,
    )
