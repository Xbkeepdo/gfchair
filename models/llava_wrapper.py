        ...
        if use_dgst:
            out, captures = run_forward_with_dgst_captures(
                ...
                retain_attention_updates=(
                    ...
                    or bool(cfg_dgst_t.get("jffn_validate_linearity", False))
                ),
            ...
            if _is_four_gate_mode(cfg_dgst_t):
                dgst_t_result = compute_four_gate_dgst_batch_from_captures(
                    ...
                    spatial_cost_semantic_weight=float(
                        cfg_dgst_t.get("spatial_cost_semantic_weight", 1.0)
                    ),
                    spatial_cost_distance_weight=float(
                        cfg_dgst_t.get("spatial_cost_distance_weight", 1.0)
                    ),
                    ...
                    jffn_entropy_beta_by_layer=cfg_dgst_t.get(
                        "jffn_entropy_beta_by_layer"
                    ),
                    jffn_max_increment_gib=float(
                        cfg_dgst_t.get("jffn_max_increment_gib", 1.5)
                    ),
                    jffn_chunk_size=cfg_dgst_t.get("jffn_chunk_size"),
                    jffn_force_full_parallel=bool(
                        cfg_dgst_t.get("jffn_force_full_parallel", False)
                    ),
                    jffn_validate_linearity=bool(
                        cfg_dgst_t.get("jffn_validate_linearity", False)
                    ),
                    jffn_finite_difference_eta=cfg_dgst_t.get(
                        "jffn_finite_difference_eta"
                    ),
                    jffn_parity_chunk_size=cfg_dgst_t.get(
                        "jffn_parity_chunk_size"
                    ),
    ...
    def _extract_token_features_batch_full_response(
        ...
        if use_dgst:
            ...
            dgst_results = compute_dgst_t_batch_from_captures(
                ...
                four_gate_spatial_cost_semantic_weight=float(
                    cfg_dgst_t.get("spatial_cost_semantic_weight", 1.0)
                ),
                four_gate_spatial_cost_distance_weight=float(
                    cfg_dgst_t.get("spatial_cost_distance_weight", 1.0)
                ),
                compute_prompt_cafe=bool(
                    cfg_dgst_t.get("compute_prompt_cafe", False)
                ...
                jffn_second_round_diagnostics=bool(
                    cfg_dgst_t.get("jffn_second_round_diagnostics", False)
                ),
                jffn_second_round_only=bool(
                    cfg_dgst_t.get("jffn_second_round_only", False)
                ),
                jffn_union_target_distributions=cfg_dgst_t.get(
                    "jffn_union_target_distributions"
                ),
