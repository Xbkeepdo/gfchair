...
class Qwen3VLWrapper(BaseLVLMWrapper):
    ...
    def extract_token_features_batch(
        ...
        if cfg_dgst_t is not None and bool(
            cfg_dgst_t.get("jffn_qwen_shared_forward", False)
        ):
            from models.qwen_shared_caption import extract_qwen_shared_caption

            prompt_inputs = self._prompt_inputs(
                image, self.resolve_prompt(prompt)
            )
            return extract_qwen_shared_caption(
                wrapper=self,
                ...
                cfg_dgst_t=cfg_dgst_t,
                requirements=self.resolve_extraction_requirements(
                    requirements, dgst_enabled=True
                ),
                model_name="Qwen3-VL",
            )
