"""Evaluate native SVAR using LLaVA layers 5--18 and proportional ranges."""
...
REFERENCE_LAYERS = 32
REFERENCE_START = 5
REFERENCE_END_EXCLUSIVE = 19
OUT = ROOT / "outputs/svar_llava_5_18_proportional_811_v1"
...
def proportional_range(total_layers):
    """Map LLaVA's zero-based inclusive 5--18 range to a model's depth."""
    start = int(round(total_layers * REFERENCE_START / REFERENCE_LAYERS))
    end = int(round(total_layers * REFERENCE_END_EXCLUSIVE / REFERENCE_LAYERS))
    return start, end
...
def prepare(model_name):
    ...
    start, end = proportional_range(total_layers)
    features = np.stack([
...
def run_model(model_name, device):
    ...
    protocol = {
        "schema": "native-svar-llava-5-18-proportional-811-v1",
        "model": model_name,
        ...
        "status": "Exploratory proportional-layer follow-up after test-set access.",
    }
...
def summarize_all():
    ...
    atomic_json_save(
        {"schema": "native-svar-llava-5-18-proportional-811-v1", "summaries": summaries},
        OUT / "summary.json",
    )
