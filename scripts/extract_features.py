...
def _merge_feature_parts(
    *,
    ...
    def merge_one(path: str) -> None:
        source_seen: Counter[tuple] = Counter()
        for feat in load_pkl(path):
            key = _feature_key(feat)
            source_seen[key] += 1
            # Root and worker parts can contain the same records during
            # resume.  Keep the maximum multiplicity from any one artifact:
            # unlike set-based deduplication, this preserves distinct InsLen
            # samples that intentionally share image/index/normalized word.
            if source_seen[key] > seen[key]:
                merged.append(feat)
                seen[key] += 1

    if resume and os.path.exists(output_path):
        merge_one(output_path)
...
def _controlled_keys_by_image(
    output_path: str,
    part_paths: list[str],
) -> dict[int, Counter[tuple[int, str, int]]]:
    """Collect exact controlled-token keys already present per image."""

    keys: dict[int, Counter[tuple[int, str, int]]] = {}
    for path in [output_path, *part_paths]:
        if not os.path.exists(path):
            continue
        path_keys: dict[int, Counter[tuple[int, str, int]]] = {}
        for feat in load_pkl(path):
            ...
            path_keys.setdefault(image_id, Counter())[key] += 1
        for image_id, counts in path_keys.items():
            keys.setdefault(image_id, Counter())
            keys[image_id] |= counts
    return keys
...
def _expected_controlled_keys(
    ...
    expected: Counter[tuple[int, str, int]] = Counter()
...
def _feature_provenance(
    *,
    ...
    if str(locator) != INSLEN_TARGET_PROTOCOL:
        raise RuntimeError(
            "Refusing feature extraction from a non-InsLen token locator "
            f"({locator!r}); expected {INSLEN_TARGET_PROTOCOL!r}."
        )
    if str(sample_unit) != INSLEN_SAMPLE_UNIT:
        raise RuntimeError(
            "Refusing feature extraction from an incompatible sample unit "
            f"({sample_unit!r}); expected {INSLEN_SAMPLE_UNIT!r}."
        )
    ...
    incompatible_rows = []
    for image_id, row in labeling_payload.items():
        protocol = row.get("labeling_protocol") or {}
        if (
            not isinstance(protocol, Mapping)
            or str(protocol.get("primary_locator")) != INSLEN_TARGET_PROTOCOL
            or str(protocol.get("sample_unit")) != INSLEN_SAMPLE_UNIT
            or str(protocol.get("upstream_commit")) != INSLEN_UPSTREAM_COMMIT
        ):
            incompatible_rows.append(str(image_id))
    if incompatible_rows:
        raise RuntimeError(
            "Refusing feature extraction because labeling rows do not declare "
            f"the gfchair InsLen protocol; examples={incompatible_rows[:5]}"
        )
    ...
    return {
        ...
        "labeling_upstream_repository": INSLEN_UPSTREAM_REPOSITORY,
        "labeling_upstream_commit": INSLEN_UPSTREAM_COMMIT,
