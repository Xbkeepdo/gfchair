...
def _register_four_gate_aliases() -> None:
    ...
    for method in methods:
        ...
        cost_risk_specs = {
            ...
            f"{method}_risk_sqrt_matched_state_plus_spatial": (
                f"dgst_t_{method}_risk_sqrt_{state_name}_plus_spatial_per_layer"
            ),
            f"{method}_risk_sqrt_matched_state_plus_spatial_"
            "capped_topmass_085": (
                f"dgst_t_{method}_risk_sqrt_{state_name}_plus_spatial_"
                "capped_topmass_085_per_layer"
            ),
        ...
        for block, feature_key in cost_risk_specs.items():
            ...
            FEATURE_KEYS[block] = feature_key
        hpre_source_specs = {
            f"{method}_source_hpre_cos_risk_sqrt_matched_state": (
                f"dgst_t_{method}_source_hpre_cos_risk_sqrt_"
                f"{state_name}_per_layer"
            ),
            f"{method}_source_hpre_cos_risk_cosine_matched_state": (
                f"dgst_t_{method}_source_hpre_cos_risk_cosine_"
                f"{state_name}_per_layer"
            ),
            f"{method}_source_hpre_cos_risk_sqrt_matched_state_"
            "capped_topmass_085": (
                f"dgst_t_{method}_source_hpre_cos_risk_sqrt_{state_name}_"
                "capped_topmass_085_per_layer"
            ),
        }
        for block, feature_key in hpre_source_specs.items():
            FEATURE_ALIASES[block] = block
            FEATURE_KEYS[block] = feature_key
        FEATURE_ALIASES[
        ...
        FEATURE_ALIASES[
            f"{method}_risk_sqrt_cosine_matched_state_plus_spatial"
        ] = f"{method}_risk_sqrt_matched_state_plus_spatial"
    ...
    for scope_prefix in ("vp", "vpend"):
        for method in methods:
            ...
            specs = {
                ...
                f"{scoped_method}_risk_sqrt_matched_state_plus_spatial": (
                    f"dgst_t_{scoped_method}_risk_sqrt_{state_name}_"
                    "plus_spatial_per_layer"
                ),
                f"{scoped_method}_risk_sqrt_matched_state_plus_spatial_"
                "capped_topmass_085": (
                    f"dgst_t_{scoped_method}_risk_sqrt_{state_name}_"
                    "plus_spatial_capped_topmass_085_per_layer"
                ),
            ...
            for block, feature_key in specs.items():
                ...
                if feature_key is not None:
                    FEATURE_KEYS[block] = feature_key
            hpre_source_specs = {
                f"{scoped_method}_source_hpre_cos_"
                "risk_sqrt_matched_state": (
                    f"dgst_t_{scoped_method}_source_hpre_cos_risk_sqrt_"
                    f"{state_name}_per_layer"
                ),
                f"{scoped_method}_source_hpre_cos_"
                "risk_cosine_matched_state": (
                    f"dgst_t_{scoped_method}_source_hpre_cos_risk_cosine_"
                    f"{state_name}_per_layer"
                ),
                f"{scoped_method}_source_hpre_cos_"
                "risk_sqrt_matched_state_capped_topmass_085": (
                    f"dgst_t_{scoped_method}_source_hpre_cos_risk_sqrt_"
                    f"{state_name}_capped_topmass_085_per_layer"
                ),
            }
            for block, feature_key in hpre_source_specs.items():
                FEATURE_ALIASES[block] = block
            ...
            FEATURE_ALIASES[
                f"{scoped_method}_risk_sqrt_cosine_matched_state_plus_spatial"
            ] = f"{scoped_method}_risk_sqrt_matched_state_plus_spatial"
...
def _four_gate_risk_block(feat: dict, block: str) -> np.ndarray:
    ...
    elif cost_mode == "sqrt_cosine_matched_state_plus_spatial":
        _scope_prefix, base_method = _four_gate_scope_and_method(method)
        state_name = "hmid" if base_method.startswith("hmid_") else "hpre"
        key = f"dgst_t_{method}_risk_sqrt_{state_name}_plus_spatial_per_layer"
