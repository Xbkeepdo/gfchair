"""Fixed temperature 0.02 control for the saved O_FFN cosine study."""
import argparse
import csv
import json
import sys
...
OUT = base.OUT / 'cohort500/temperature002'
NAME = 'J_output_tau002'


def temperature_js(cosines, target, temperature=.02):
...
def run(model, device):
    ...
    lookup, pmax, source_hashes = {}, {1.:[], .02:[]}, {}
    for i in old['cohort']['split']['train']+old['cohort']['split']['test']:
        ...
        for t in row['positions']:
            ...
            new, audit2=temperature_js(t['cosines'],target,.02)
            lookup[key]=(one,new)
            pmax[1.].append(audit1['p_max']);pmax[.02].append(audit2['p_max'])
    ...
    protocol=dict(model=model,temperature=.02,definition='JS(softmax(cos(e_m,actual O_FFN)/0.02),T), all visual tokens',
    ...
    value=dict(model=model,temperature=.02,status='COMPLETE',summary=result,baseline_tau1=oldsummary,
        baseline_tau02=json.loads((base.OUT/'cohort500/temperature02'/model/'summary.json').read_text())['summary'],
...
def summarize():
    ...
    lines=['# O_FFN cosine–JS：固定温度0.02', '', '共享500图（400/100），同一cosine/标签/MLP与seeds43/44/45，AUROC/AP为三seed概率ensemble。', '',
           '| 模型 | tau=1 AUROC / HALL-AUPR | tau=0.2 AUROC / HALL-AUPR | tau=0.02 AUROC / HALL-AUPR |','|---|---:|---:|---:|']
    for ax,(m,r) in zip(axes.flat,results.items()):
        cells=[]
        for key in ['baseline_tau1','baseline_tau02','summary']:
    ...
    fig.suptitle('JS(softmax(cos(e_m, O_FFN) / 0.02), T)')
...
def self_check():
    c=torch.tensor([[.02,-.04,.08]],dtype=torch.float64);t=torch.tensor([[.2,.3,.5]],dtype=torch.float64)
    p=torch.softmax(c/.02,-1);mid=(p+t)/2
