"""Optional provenance and required completeness checks for training features."""
...
def load_validated_training_features(
    *,
    ...
    """Load a complete feature artifact, optionally checking its manifest."""
    ...
    if manifest_validation_enabled(config):
        ...
        actual_provenance = load_json(str(manifest_path))
        expected_provenance = expected_feature_provenance(
        ...
        if actual_provenance != expected_provenance:
            actual = (
                actual_provenance
                if isinstance(actual_provenance, Mapping)
                else {}
            )
            differing_keys = sorted(
                key
                for key in set(actual) | set(expected_provenance)
                if actual.get(key) != expected_provenance.get(key)
            )
            raise RuntimeError(
                f"Refusing to train from {path}: feature provenance mismatch; "
                f"differing_keys={differing_keys[:10]}."
            )
