...
from utils.inslen_targeting import INSLEN_SAMPLE_UNIT, INSLEN_TARGET_PROTOCOL
...
class LabelingConfigTests(unittest.TestCase):
    ...
    def test_gfchair_config_selects_only_the_inslen_target_protocol(self) -> None:
        config = load_config("configs/model_configs_inslen_official_target.yaml")
        labeling = get_labeling_cfg(config)
        self.assertEqual(labeling["sample_unit"], INSLEN_SAMPLE_UNIT)
        self.assertEqual(labeling["primary_locator"], INSLEN_TARGET_PROTOCOL)
        self.assertFalse(labeling["save_svar_official_samples"])
