...
from utils.io_utils import load_pkl, save_json, save_pkl
...
class StageResumeTests(unittest.TestCase):
    ...
    def test_complete_labeling_resumes_when_manifest_validation_is_disabled(self) -> None:
        samples = [{"image_id": 1}]
        generations = {
            "1": {
                "generated_text": "a chair",
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            # Disabling the sidecar manifest does not disable the structural
            # ground-truth validation itself.
            ground_truth_path.write_text(
                json.dumps({"image_id": 2, "objects": []}) + "\n",
                encoding="utf-8",
            )
    ...
    def test_resume_preserves_duplicate_inslen_sample_multiplicity(self) -> None:
        record = {
            "image_id": 1,
            "response_token_idx": 2,
        ...
        labeling = {
            1: {
                "object_token_spans": [
                    {"word": "skateboard", "token_indices": [2], "label": 1},
                    {"word": "skateboard", "token_indices": [2], "label": 1},
                ]
            }
        }
        with tempfile.TemporaryDirectory() as directory:
            output = Path(directory) / "features.pkl"
            save_pkl([record], str(output))
            pending, complete = _pending_samples_for_resume(
                samples=[{"image_id": 1}],
                labeling_results=labeling,
                root_output_path=str(output),
            ...
            save_pkl([record, dict(record)], str(output))
            pending, complete = _pending_samples_for_resume(
                samples=[{"image_id": 1}],
                labeling_results=labeling,
                root_output_path=str(output),
    ...
    def test_merge_uses_multiset_union_for_duplicate_inslen_samples(self) -> None:
        record = {
            "image_id": 1,
            "response_token_idx": 2,
            "token_str": "skateboard",
        ...
        other_label = {**record, "label": 0}
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            output = root / "features.pkl"
            part = root / "features.part0.pkl"
            save_pkl([record], str(output))
            save_pkl([record, dict(record), other_label], str(part))
            ...
            merged = load_pkl(str(output))
            self.assertEqual(len(merged), 3)
            self.assertEqual([row["label"] for row in merged].count(1), 2)
            self.assertEqual([row["label"] for row in merged].count(0), 1)
            ...
            self.assertEqual(len(load_pkl(str(output))), 3)
    ...
    def test_feature_provenance_tracks_changed_labeling_content_and_locator(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            labeling_payload = {
                "1": {
            ...
            self.assertNotEqual(
                provenance["labeling_sha256"],
                changed_content["labeling_sha256"],
            )
            ...
            with self.assertRaisesRegex(RuntimeError, "non-InsLen token locator"):
                _feature_provenance(
                    artifact_family="root",
                    model_key="model",
                    model_cfg={"hf_name": "unused"},
    ...
    def test_feature_provenance_requires_matching_generations(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "labeling.json").write_text(
                json.dumps(
            ...
            with self.assertRaisesRegex(RuntimeError, "generations.json"):
                _feature_provenance(
                    artifact_family="root",
                    model_key="model",
                    model_cfg={"hf_name": "unused"},
