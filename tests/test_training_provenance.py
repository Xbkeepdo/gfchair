...
class TrainingProvenanceTests(unittest.TestCase):
    ...
    def test_disabled_manifest_validation_accepts_artifact_without_manifest(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            config, splits, labeling = self._fixture(root)
            config["run"]["validate_manifests"] = False
            path = root / "features.pkl"
            save_pkl(self._records(labeling), str(path))
            ...
            self.assertEqual(len(loaded), 10)
            self.assertFalse((root / "features_manifest.json").exists())
